import { ConflictException, Inject, Injectable } from '@nestjs/common';
import { and, eq } from 'drizzle-orm';
import { AuthenticatedUser } from '../../common/interfaces/authenticated-user.interface';
import { DATABASE_CONNECTION, Database } from '../../database/database.module';
import { permitTypes } from '../../database/schema';
import { AuditService } from '../logging/audit.service';
import { CreatePermitTypeDto } from './dto/create-permit-type.dto';
import { MasterDataCacheService } from './master-data-cache.service';
import { MasterDataLogService } from './master-data-log.service';
import { ReferenceIntegrityService } from './reference-integrity.service';

@Injectable()
export class PermitTypeService {
  constructor(
    @Inject(DATABASE_CONNECTION) private readonly db: Database,
    private readonly cacheService: MasterDataCacheService,
    private readonly auditService: AuditService,
    private readonly logService: MasterDataLogService,
    private readonly referenceIntegrity: ReferenceIntegrityService,
  ) {}

  async create(dto: CreatePermitTypeDto, user: AuthenticatedUser) {
    const tenantId = this.referenceIntegrity.requireTenant(user);

    try {
      const [row] = await this.db
        .insert(permitTypes)
        .values({
          tenantId,
          code: dto.code.trim(),
          name: dto.name.trim(),
          description: dto.description,
          defaultAttributes: dto.defaultAttributes ?? null,
          isActive: dto.isActive ?? true,
          createdBy: user.id,
          updatedBy: user.id,
        })
        .returning();

      await this.afterMutation(tenantId, user, 'permit-type.created', row.id);
      return row;
    } catch (error) {
      if (this.isUniqueViolation(error)) {
        throw new ConflictException(`Permit type code '${dto.code}' already exists`);
      }
      throw error;
    }
  }

  async findAll(user: AuthenticatedUser) {
    const tenantId = this.referenceIntegrity.requireTenant(user);
    const cached = await this.cacheService.get<(typeof permitTypes.$inferSelect)[]>(
      tenantId,
      'permit-types',
    );
    if (cached) {
      return cached;
    }

    const rows = await this.db
      .select()
      .from(permitTypes)
      .where(eq(permitTypes.tenantId, tenantId));

    await this.cacheService.set(tenantId, 'permit-types', rows);
    return rows;
  }

  async remove(id: string, user: AuthenticatedUser) {
    const tenantId = this.referenceIntegrity.requireTenant(user);
    await this.referenceIntegrity.assertPermitTypeNotReferenced(tenantId, id);

    const [row] = await this.db
      .delete(permitTypes)
      .where(and(eq(permitTypes.id, id), eq(permitTypes.tenantId, tenantId)))
      .returning();

    if (!row) {
      throw new ConflictException('Permit type not found');
    }

    await this.afterMutation(tenantId, user, 'permit-type.deleted', id);
    return row;
  }

  private async afterMutation(
    tenantId: string,
    user: AuthenticatedUser,
    action: string,
    entityId: string,
  ) {
    await this.cacheService.invalidate(tenantId, 'permit-types');
    await this.auditService.log({
      action,
      entityType: 'permit_type',
      entityId,
      userId: user.id,
      tenantId,
    });
    this.logService.logEvent({
      action,
      tenantId,
      userId: user.id,
      entityType: 'permit_type',
      entityId,
    });
  }

  private isUniqueViolation(error: unknown): boolean {
    return error instanceof Error && error.message.includes('unique');
  }
}
