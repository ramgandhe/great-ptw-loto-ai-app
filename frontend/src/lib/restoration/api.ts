...
import type {
  EquipmentRestoration,
  LockRemoval,
  LototoHistoryEntry,
  RestorationDetail,
  RestorationVerificationPayload,
...
export function restoreEquipment(executionId: string, payload: RestoreEquipmentPayload) {
  return fetchApi<EquipmentRestoration>(
    `/isolation-executions/${executionId}/restoration/equipment`,
    {
      method: "POST",
...
export function recordRestorationVerification(
  executionId: string,
  payload: RestorationVerificationPayload,
) {
  return fetchApi<unknown>(`/isolation-executions/${executionId}/restoration/verifications`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
