...
/** SHA-256; Web Crypto is missing on HTTP origins that are not localhost. */
...
function sha256Fallback(bytes: Uint8Array): Uint8Array {
  ...
  const blockCount = ((withPad + 63) >> 6) << 6;
  const padded = new Uint8Array(blockCount);
  padded.set(bytes);
  padded[bytes.length] = 0x80;
  const view = new DataView(padded.buffer);
  view.setUint32(blockCount - 4, bitLen >>> 0);
  view.setUint32(blockCount - 8, Math.floor(bitLen / 0x100000000));
