...
- **Linear access from Cloud Agents:** Interactive Linear MCP OAuth (`mcp_auth`) only works in the Cursor desktop IDE and does **not** propagate to Cloud Agent VMs. After desktop auth, Cloud Agents may still see Linear as `needsAuth`. Prefer a `LINEAR_API_KEY` secret (Linear → Settings → API → Personal API keys) and use Linear’s GraphQL API (`https://api.linear.app/graphql`) for issue reads/updates when MCP is unavailable. Do not block implementation solely on Linear MCP if the API key is present.
